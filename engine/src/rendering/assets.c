...
int assets_load(void)
{
    ...
    unsigned long long default_texture_width = 256;
    unsigned long long default_texture_height = 256;
    unsigned int *default_texture_pixels = malloc(default_texture_width * default_texture_height * sizeof(unsigned int));

    if (!default_texture_pixels)
    {
        printf("Error: Couldn't allocate default texture pixels\n");

        return 1;
    }

    memset(default_texture_pixels, -1, default_texture_width * default_texture_height * sizeof(unsigned int));
    assets.default_texture = texture_create((int)default_texture_width, (int)default_texture_height, 4, default_texture_pixels);
