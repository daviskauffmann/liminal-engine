...
void main()
{
    ...
    // directional light
    result += calc_directional_light(directional_light, normal, view_direction);

    // point lights
    for (int i = 0; i < 4; i++)
    {
        result += calc_point_light(point_lights[i], normal, view_direction);
    }

    // spot light
    result += calc_spot_light(spot_light, normal, view_direction);

    // emission
    result += vec3(texture(material.emission, vertex.uv)) * material.glow;
